...
/**
 * Writer for arbitrary data using RoaringBitmap with type value encoded in upper bits.
 * Uses 4 bits (28-31) to encode type values 0-15.
 */
public class ArbitraryDataBitmapWriter implements ITileTypeMapWriter {
    private final RoaringBitmap bitmap;

    public ArbitraryDataBitmapWriter() {
    ...
    public synchronized void setTileType(int x, int y, int plane, byte type) {
        ...
        if ((type & 0b0001) != 0) bitmap.add(packed | ArbitraryDataBitmap.DATA_BIT_0);
        if ((type & 0b0010) != 0) bitmap.add(packed | ArbitraryDataBitmap.DATA_BIT_1);
        if ((type & 0b0100) != 0) bitmap.add(packed | ArbitraryDataBitmap.DATA_BIT_2);
        if ((type & 0b1000) != 0) bitmap.add(packed | ArbitraryDataBitmap.DATA_BIT_3);
    }
